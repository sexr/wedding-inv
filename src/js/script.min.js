const BASE_INVITATION_URL='https://inv.widasari.web.id';document.addEventListener('DOMContentLoaded',()=>{const urlParams=new URLSearchParams(window.location.search);const guestName=urlParams.get('to');let dynamicDescription="Dengan hormat, kami mengundang Anda untuk merayakan pernikahan Mamat & Duni.";const dearGuestElement=document.getElementById('dear-guest');if(guestName){dearGuestElement.textContent=`Dear: ${decodeURIComponent(guestName)}`;dynamicDescription=`Undangan pernikahan Mamat & Duni untuk ${decodeURIComponent(guestName)}. Kami mengundang Anda untuk merayakan hari bahagia kami.`}else{dearGuestElement.textContent=`Dear: Tamu Undangan`}
